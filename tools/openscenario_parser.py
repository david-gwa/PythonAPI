...
from scenario.timer import SimulationTimeCondition
...
class OpenScenarioParser(object):
    ...
    def convert_position_to_transform(position):
        ...
        if position.find('World') is not None:
            ...
            # return lgsvl.Transform(lgsvl.Location(x=x, y=y, z=z), lgsvl.Rotation(yaw=yaw, pitch=pitch, roll=roll))
            return lgsvl.Transform(lgsvl.Vector(x,z,y), lgsvl.Vector(pitch,-yaw,-roll))
                ...
                for entity in triggering_entities.iter('Entity'):
                    for actor in actor_list:
                        if entity.attrib.get('name', None)=='hero':
                            if actor.agent_type==lgsvl.AgentType.EGO:
                                trigger_actor=actor
                        elif entity.attrib.get('name', None) == actor.name:
            ...
            for entity_condition in condition.find('ByEntity').iter('EntityCondition'):
                if entity_condition.find('EndOfRoad') is not None:
                    ...
                    if condition_target.find('Position'):
                        ...
                        for actor in actor_list:
                            if ttc_condition.attrib.get('entity', None) == actor.name:
                    ...
                    if distance_condition.attrib.get('type') == "inertial":
                        for actor in actor_list:
                            print(actor.name)
                            if distance_condition.attrib.get('entity', None) == actor.name:
        ...
        if action.find('Global') is not None:
            ...
            if private_action.find('Longitudinal') is not None:
                ...
                if private_action.find('Speed') is not None:
                    ...
                    #                       name=maneuver_name)
                    # tsui debug 1
                    # atomic= SetToVelocity(actor, target_speed, delay_time=0.0, name="Acceleration")
                    # tsui debug 2
                    atomic = FollowClosestLane(actor, target_speed)
