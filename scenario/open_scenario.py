...
from tools.openscenario_parser import OpenScenarioParser
...
class Decorator(py_trees.behaviour.Behaviour):
    ...
    def tick(self):
        ...
        """
        print("%s.tick()" % self.decorated.name)
...
class StoryElementStatusToBlackboard(Decorator):
    ...
    def __init__(
        ...
        print('child class StoryElementStatusToBlackboard')
        print(child)
        print(story_element_type)
        print(element_name)
...
class OpenScenario(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, config_file, sim=None, debug_mode=False, criteria_enable=True, timeout=300):
        """
        ...
        super(OpenScenario, self).__init__("OpenScenario", ego_vehicles=ego_vehicles, 
                                            simendtime=timeout, config=config,
    ...
    def _initialize_actors(self,config):
        ...
        for actor in config.other_actors:
            ...
            print(npc_transform)
            new_actor = ServerActorPool.request_new_npc(actor.rolename, npc_transform)
            ...
            for sequence in act.iter("Sequence"):
                ...
                for actor in sequence.iter("Actors"):
                    for entity in actor.iter("Entity"):
                        for k, _ in enumerate(joint_actor_list):
                            if entity.attrib.get('name', None)=='hero':
                                if joint_actor_list[k].agent_type==lgsvl.AgentType.EGO:
                                    actor_ids.append(k)
                                    break
                            elif entity.attrib.get('name', None) ==  joint_actor_list[k].name:
                                actor_ids.append(k)
                                break
                ...
                for maneuver in sequence.iter("Maneuver"):
                    ...
                    for event in maneuver.iter("Event"):
                        print('event name is: '+ event.attrib.get('name'))
                        ...
                        for child in event.iter():
                            print('event child tag is: '+child.tag)
                            if child.tag == "Action":
