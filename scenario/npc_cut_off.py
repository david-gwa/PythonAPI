...
NPC_CUT_OFF_SCENARIO = ["NpcCutOff"]

class NpcCutOff(BasicScenario):
    #TODO: parameters pass-in, not hard-code 
    def __init__(self, ego, agents,config = None,sim=None):
        self.sim = sim
        self._npc_drive_distance = 200 
        self._npc_speed = 10
        super(NpcCutOff, self).__init__("NpcCutOff", ego, agents,config=config) 
        input("NPC_CUTOFF scenario hold on...")
    ...
    def _create_behavior(self):
        npc_travel_first_half =  py_trees.composites.Parallel("npc cut_off",     
        ...
        sequence = py_trees.composites.Sequence("npc cut_off behavior")
